...
class IWSLT217(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        if self.config.is_multilingual:
            ...
            # bi_url = BI_URL.format(source=source, target=target) # 注释掉原来的网络 URL
            local_zip_path = "./en-de.zip"  # 直接指向我们上传的本地压缩包
            dl_dir = dl_manager.extract(local_zip_path) # 使用 extract 函数，而不是 download_and_extract
